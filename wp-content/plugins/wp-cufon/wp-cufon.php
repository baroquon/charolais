...
Plugin URI: http://www.tobias-battenberg.de/wp-cufon
...
?>
    ...
    <!-- WP-Cufon Delay Fixes enabled! (beta) -->
        <!--[if IE]> 
...
<?php } // end function
add_action('wp_head', 'wpcufon_init');
...
function wp_cufon_ietag() {
    ...
    <?php if(get_option("cufon_delay_fix") == "1"){ ?> <script type="text/javascript">Cufon.refresh();</script><?php } ?>
<?php }
...
function wpcufon_set_font() {	
	?>
    <p><strong>The plugin automatically found these font-files for Cuf&oacute;n:</strong><br />
    <small>Only the fonts you want to replace get automatically activated - enter them with there fontName in the replacement-field!</small>
    </p>	
    <p>
    <ul>
    <?php
...
$first_value = "Cufon.replace('h1', { fontFamily: 'Vegur' });";
add_option("cufon_replacement_script",stripslashes($first_value));
add_option("cufon_jquery","");
add_option("cufon_delay_fix","");

function wpcufon_set_font_replacements(){
?>
   <strong> <p>Please enter your source for the replacements:</p></strong>
...
<form name="form1" method="post" action="">
    ...
    <p><label><input name="cufon_delay_fix" type="checkbox" value="1" <?php if(get_option("cufon_delay_fix") == "1"){ echo"checked"; }?>  /> Enable some "Hacks" to get rid of Cuf&oacute;n's delayed appearence on your site <small>(rude and beta)</small></label></p>
    
    <p>&nbsp;</p>
    
    <p><input type="submit" value=" Save Settings " /></p>
    <input name="action" value="saveDB" type="hidden" />
</form>
